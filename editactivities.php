...
require_once(dirname(__FILE__) . '/notebookdec_edit_activity_form.php');
...
// Get the notebookdec activity, course, etc from the problem.
...
if ($course->id) {
  ...
  error('Could not find the course / notebookdec activity!');
}

// Moodley goodness.
require_login($course, true, $cm);
$context = get_context_instance(CONTEXT_MODULE, $cm->id);
add_to_log($course->id, 'notebookdec', 'view', "editactivities.php?sid=$sid", $session->name, $cm->id);
...
$PAGE->set_url('/mod/notebookdec/editactivities.php', array('sid' => $sid, 'aid' => $aid));
$PAGE->set_title(format_string("Editing Activities."));
$PAGE->set_heading(format_string($course->fullname));
$PAGE->set_context($context);
$PAGE->add_body_class('notebookdec-activity-edit-form');
...
if ($aid != 0) {
  ...
  $activity = $DB->get_record('notebookdec_activities', array('sid' => $sid, 'id' => $aid));
...
// Load the form.
$mform = new notebookdec_edit_activity_form("/mod/notebookdec/editactivities.php?sid=$sid&aid=$aid", array('activities' => $activities, 'this_activity' => $activity));
...
if ($mform->is_cancelled()) {
  ...
  if ($results = $mform->get_data()) {
    ...
    if ($aid == 0) {
      ...
      $new_record = $DB->insert_record('notebookdec_activities', $activity);
    }
    else {
      // We're updaing an existing activity.
      $activity->name = $results->name;
      $updated_record = $DB->update_record('notebookdec_activities', $activity);
    }
...
echo "<div class='notebookdec-activity-wrapper'>";

echo "<div class='notebookdec-activity-pager'>";
echo "<h4>Select an activityto edit,<br /> or click \"Add New\" to create a new activity.</h4>";
echo "<ul>";
foreach ($activities as $activity) {
  $class = ($aid == $activity->id) ? "class=\"notebookdec-pager-current\"" : ""; 
  echo '<li ' . $class . '><a href="' . $CFG->wwwroot . '/mod/notebookdec/editactivities.php?sid=' . $activity->sid . '&amp;aid=' . $activity->id . '">' . $activity->name . '</a></li>';
}
$class = (!$aid) ? ' class="notebookdec-pager-current" ' : "";
echo '<li' . $class . '><a href="' . $CFG->wwwroot . '/mod/notebookdec/editactivities.php?sid=' . $session->id . '">Add New</a></li>';
echo "</ul>";
echo "</div>";

echo "<div class='notebookdec-manage-form-wrapper'>";
if ($aid) echo "<p class='notebookdec-delete-link'><a href='deleteactivity.php?aid=$aid'>Delete this sample</a></p>";
...
echo '<span class="notebookdec-back-link-box"><a href="view.php?id=' . $cm->id . '">Back to the notebook</a></span>';
echo "</div>";
echo "</div>";
