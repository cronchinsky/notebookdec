...
require_once(dirname(__FILE__) . '/notebookdec_edit_probe_form.php');
...
add_to_log($course->id, 'notebookdec', 'view', "editprobes.php?sid=$sid", $session->name, $cm->id);
...
$PAGE->set_url('/mod/notebookdec/editprobes.php', array('sid' => $sid, 'pid' => $pid));
$PAGE->set_title(format_string("Editing Probes."));
$PAGE->set_heading(format_string($course->fullname));
$PAGE->set_context($context);
$PAGE->add_body_class('notebookdec-probe-edit-form');
...
if ($pid != 0) {
  // Get a probe to load 
  $probe = $DB->get_record('notebookdec_probes', array('id' => $pid));
...
// Load the form.
$mform = new notebookdec_edit_probe_form("/mod/notebookdec/editprobes.php?sid=$sid&pid=$pid", array('probes' => $probes, 'this_probe' => $probe));
...
if ($mform->is_cancelled()) {
  ...
  if ($results = $mform->get_data()) {
    ...
    if ($pid == 0) {
      ...
      $new_record = $DB->insert_record('notebookdec_probes', $probe);
    }
    else {
      // We're updaing existing work.
      $probe->name = $results->probename;
      $updated_record = $DB->update_record('notebookdec_probes', $probe);
    }
...
echo "<div class='notebookdec-probe-wrapper'>";

echo "<div class='notebookdec-probe-pager'>";
echo "<h4>Select a probe to edit,<br /> or click \"Add New\" to create a new probe.</h4>";
echo "<ul>";
foreach ($probes as $probe) {
  $class = ($pid == $probe->id) ? "class=\"notebookdec-pager-current\"" : ""; 
  echo '<li ' . $class . '><a href="' . $CFG->wwwroot . '/mod/notebookdec/editprobes.php?sid=' . $probe->sid . '&amp;pid=' . $probe->id . '">' . $probe->name . '</a></li>';
}
$class = (!$pid) ? ' class="notebookdec-pager-current" ' : "";
echo '<li' . $class . '><a href="' . $CFG->wwwroot . '/mod/notebookdec/editprobes.php?sid=' . $session->id . '">Add New</a></li>';
echo "</ul>";
echo "</div>";

echo "<div class='notebookdec-manage-form-wrapper'>";
if ($pid) echo "<p class='notebookdec-delete-link'><a href='deleteprobe.php?pid=$pid'>Delete this probe [not implemented yet]</a></p>";
