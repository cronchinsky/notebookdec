...
require_once(dirname(__FILE__) . '/notebookdec_edit_session_form.php');
...
add_to_log($course->id, 'notebookdec', 'view', "editsession.php?sid=$sid&nid=$nid", $session->name, $cm->id);
...
$PAGE->set_url('/mod/notebookdec/editsession.php', array('sid' => $sid));
$PAGE->set_title(format_string("Editing Session $session->name"));
$PAGE->set_heading(format_string($course->fullname));
$PAGE->set_context($context);
$PAGE->add_body_class('notebookdec-session-edit-form');
...
// Load the form.
$mform = new notebookdec_edit_session_form("/mod/notebookdec/editsession.php?sid=$sid&nid=$nid", array('session' => $session));
...
if ($mform->is_cancelled()) {
  ...
  if ($results = $mform->get_data()) {
      ...
      $updated_record = $DB->update_record('notebookdec_sessions', $session);
